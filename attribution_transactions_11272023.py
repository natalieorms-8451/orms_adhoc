...
path = "abfss://media@sa8451dbxadhocprd.dfs.core.windows.net/n510252/attribution_backfill/temp_out/10132023_11172023_txn_pull"
...
path = "abfss://media@sa8451dbxadhocprd.dfs.core.windows.net/n510252/attribution_backfill/out/10132023_11172023_txn_final_unfiltered"
txns_final.write.parquet(path)

# COMMAND ----------

txns_final_raw = spark.read.parquet( "abfss://media@sa8451dbxadhocprd.dfs.core.windows.net/n510252/attribution_backfill/out/10132023_11172023_txn_final_unfiltered")

# COMMAND ----------

txns_final_raw.select('con_upc_no').distinct().count()

# COMMAND ----------

txns_final_raw.select('trn_dt').distinct().display()
...
attr_upcs = spark.read.csv("abfss://media@sa8451dbxadhocprd.dfs.core.windows.net/n510252/attribution_backfill/input/missing_toa_upcs_11272023.csv", header=True)
...
#attr_upcs_int = attr_upcs.withColumn('attr_upc_int', f.col('upc').cast('int'))
#attr_upcs_padded = attr_upcs_int.withColumn('attr_upc_padded', f.format_string("%013d", f.col('attr_upc_int')))
attr_upcs = attr_upcs.withColumn('attr_upc_padded', f.lpad(attr_upcs.upc, 13, '0'))

# COMMAND ----------

attr_upcs.orderBy('upc', how='descending').display()
...
attr_transactions.agg(f.sum('cly_dol_am')).display()

# COMMAND ----------

attr_transactions.select('trn_dt').distinct().display()

# COMMAND ----------

attr_transactions.select('con_upc_no').distinct().count()

# COMMAND ----------

path = "abfss://media@sa8451dbxadhocprd.dfs.core.windows.net/n510252/attribution_backfill/out/10132023_11172023_txn_final_filtered"
attr_transactions.write.parquet(path)

# COMMAND ----------

#read in the output
attr_transactions = spark.read.parquet('abfss://media@sa8451dbxadhocprd.dfs.core.windows.net/n510252/attribution_backfill/out/10132023_11172023_txn_final_filtered')
